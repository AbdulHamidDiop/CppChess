...
namespace View {
    ...
    void Board::placePiece(int row, int col, std::shared_ptr<Model::ChessPiece> piece) {
    ...
    void Board::updateSquare(int row, int col) {
        std::shared_ptr<Model::ChessPiece> piece = pieces[row][col];
        if (piece != nullptr) {
            squares[row][col]->setText(piece->getUnicodeSymbol());
            squares[row][col]->setStyleSheet((row + col) % 2 == 0 ? "font-size: 40px; background-color: white;" : "font-size: 40px; background-color:#8B4513;");
        }
    ...
    bool Board::movePiece(int fromRow, int fromCol, int toRow, int toCol) {
        ...
        std::shared_ptr<Model::ChessPiece> movingPiece = pieces[fromRow][fromCol];

        if (movingPiece->isValidMove(fromRow, fromCol, toRow, toCol) && !isMoveBlocked(fromRow, fromCol, toRow, toCol) && !movingPiece->isMoveCausingCheck(fromRow, fromCol, toRow, toCol)) {
            std::shared_ptr<Model::ChessPiece> capturedPiece = pieces[toRow][toCol];

            if (capturedPiece != nullptr) {
                if (capturedPiece->isWhite() == movingPiece->isWhite()) {
                    squares[toRow][toCol]->setStyleSheet("background-color:#FF7276;");
                    return false;
                }
    ...
    bool Board::isKingInCheck(bool isWhite) const {
        ...
        for (int row = 0; row < 8; ++row) {
            for (int col = 0; col < 8; ++col) {
                std::shared_ptr<Model::ChessPiece> piece = pieces[row][col];
                if (piece != nullptr && piece->isWhite() == isWhite && dynamic_cast<Model::King*>(piece.get()) != nullptr) {
                    kingRow = row;
                    kingCol = col;
                    break;
                }
    ...
    bool Board::isMoveBlocked(int fromRow, int fromCol, int toRow, int toCol) const {
        if (pieces[fromRow][fromCol]->canLeap()) return false;
    ...
    void Board::mousePressEvent(QMouseEvent* event) {
        ...
        if (selectedRow == -1 && selectedCol == -1) {
            ...
            if ((pieces[row][col] != nullptr) && (pieces[row][col]->isWhite() != isWhiteTurn)) {
                squares[row][col]->setStyleSheet("background-color: red;");
            }

            else if (pieces[row][col] != nullptr && pieces[row][col]->isWhite() == isWhiteTurn) {
                 selectedRow = row;
                 selectedCol = col;

                for (int i = 0; i < 8; ++i) {
                    for (int j = 0; j < 8; ++j) {
                        if (pieces[row][col]->isValidMove(row, col, i, j) && !pieces[row][col]->isMoveCausingCheck(row, col, i, j) && !isMoveBlocked(row,col,i,j)) {
                            squares[i][j]->setStyleSheet("background-color: #FFD700;");
                           
                        }
                       if((pieces[i][j] != nullptr) && (pieces[i][j]->isWhite()) == (pieces[row][col]->isWhite())) {
                            squares[i][j]->setStyleSheet((i + j) % 2 == 0 ? "background-color: white" : "background-color: #8B4513");
                            squares[row][col]->setStyleSheet("background-color : green ;");
                       }
                    }
                 }
            }
